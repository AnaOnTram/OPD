...
| -------- | ----- | ------- |
| AVOID_ENABLE | 7 | ALL |
| AVOID_MARGIN | Meters that the drone will start to avoid the obstacle | Loiter Only |
| AVOID_BEHAVE | STOP or SLIDE AROUND | Loiter Only |
| AVOID_DIST_MAX | Distance the UAV starts to leaning away | Alt_hold Only |
| ATC_ANGLE_MAX | Avoidance leaning angle | Alt_hold only |


> The simple object avoidance config is only effective in either `Loiter` or `Alt_hold` mode. As mentioned two modes requires GNSS inputs for internal corrections (maintain flying stability), flying the drone indoor with the two modes might lead to instability issues as revealed in test flight.  
---
### References
